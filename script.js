...
// نستورد مكتبة Three.js لإنشاء الرسوميات ثلاثية الأبعاد، ومكتبة Multisynq لمزامنة اللعبة أونلاين.
...
// --- 2. إعداد المشهد ثلاثي الأبعاد (Scene Setup) ---
const scene = new THREE.Scene();
scene.background = new THREE.Color(0x87ceeb); // لون السماء الأزرق
scene.fog = new THREE.Fog(0x87ceeb, 30, 150); // ضباب لإضافة عمق للمشهد

const camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 1000);
const renderer = new THREE.WebGLRenderer({ antialias: true }); // antialias لتنعيم حواف الأشكال
renderer.setSize(window.innerWidth, window.innerHeight);
renderer.setPixelRatio(window.devicePixelRatio);
renderer.shadowMap.enabled = true; // تفعيل الظلال لزيادة الواقعية
document.body.appendChild(renderer.domElement); // إضافة لوحة الرسم إلى صفحة HTML

// دالة لمعالجة تغيير حجم نافذة المتصفح لتبقى اللعبة بملء الشاشة
...
// إعداد الإضاءة
const ambientLight = new THREE.AmbientLight(0xffffff, 0.7); // إضاءة محيطية عامة تضيء كل شيء بشكل متساوٍ
scene.add(ambientLight);

const directionalLight = new THREE.DirectionalLight(0xffffff, 1.5); // إضاءة تشبه الشمس، لها اتجاه وتُلقي بظلال
directionalLight.position.set(20, 30, 10);
directionalLight.castShadow = true;
directionalLight.shadow.mapSize.width = 2048; // زيادة دقة الظلال
...
ground.rotation.x = -Math.PI / 2; // تدوير الأرضية لتصبح مسطحة
ground.receiveShadow = true; // جعل الأرضية تستقبل الظلال من الأجسام الأخرى
scene.add(ground);
...
road.position.y = 0.01; // رفعه قليلاً فوق الأرضية لمنع التداخل الرسومي (Z-fighting)
...
// دالة لإنشاء شجرة في موقع محدد
function createTree(x, z) {
    const trunkGeo = new THREE.CylinderGeometry(0.15, 0.2, 1.5, 8); // جذع الشجرة
    const trunkMat = new THREE.MeshLambertMaterial({ color: 0x8B4513 });
    const trunk = new THREE.Mesh(trunkGeo, trunkMat);
    trunk.position.set(x, 0.75, z);
    trunk.castShadow = true;

    const leavesGeo = new THREE.ConeGeometry(1, 3, 8); // أوراق الشجرة على شكل مخروط
...
// --- 4. كلاس لإنشاء السيارات (SimCar) ---
// هذا الكلاس مسؤول فقط عن الشكل الرسومي للسيارة
class SimCar {
    constructor(color) {
        this.body = new THREE.Group(); // حاوية رئيسية لتسهيل التحكم بالسيارة كلها
        ...
        const wheelPositions = [{ x: 0.8, z: 1.0 }, { x: -0.8, z: 1.0 }, { x: 0.8, z: -1.0 }, { x: -0.8, z: -1.0 }];
        ...
        this.body.position.y = 0.6; // رفع السيارة فوق مستوى الأرض

        // نقطة وهمية لتتبع الكاميرا، وهي تابعة للسيارة
...
// --- 6. كلاس المحاكاة المشتركة (SharedSimulation) ---
// هذا هو "عقل" اللعبة الذي يعمل على الخادم ويقوم بحساب الفيزياء والحركة.
...
// --- 5. كلاس واجهة المستخدم والتحكم (SimInterface) ---
// هذا الكلاس يعمل عند اللاعبين، وهو مسؤول عن التحكم وعرض ما يحدث في اللعبة.
class SimInterface {
    constructor(session) {
    ...
    // إعداد التحكم عبر لوحة المفاتيح (الأسهم و WASD)
    setupKeyboardControls() {
        const keyMap = { 'KeyW': 'fwd', 'ArrowUp': 'fwd', 'KeyS': 'bwd', 'ArrowDown': 'bwd', 'KeyA': 'left', 'ArrowLeft': 'left', 'KeyD': 'right', 'ArrowRight': 'right' };
        
        const keyEventHandler = (isPressed) => (e) => {
            if (keyMap[e.code]) {
                const changed = this.input[keyMap[e.code]] !== isPressed;
                this.input[keyMap[e.code]] = isPressed;
                if (changed) this.session.input(this.input); // إرسال المدخلات للخادم فقط عند حدوث تغيير
            }
        };
    ...
    // إعداد التحكم عبر أزرار اللمس في الموبايل
    setupTouchControls() {
        const controls = { 'btn-fwd': 'fwd', 'btn-bwd': 'bwd', 'btn-left': 'left', 'btn-right': 'right' };
        for (const [id, key] of Object.entries(controls)) {
            const btn = document.getElementById(id);
            if (btn) {
                const touchEventHandler = (isPressed) => (e) => {
                    e.preventDefault();
                    const changed = this.input[key] !== isPressed;
                    this.input[key] = isPressed;
                    if (changed) this.session.input(this.input);
                };
                btn.addEventListener('touchstart', touchEventHandler(true), { passive: false });
                btn.addEventListener('touchend', touchEventHandler(false), { passive: false });
                btn.addEventListener('mousedown', touchEventHandler(true)); // لدعم الفأرة أيضاً
                btn.addEventListener('mouseup', touchEventHandler(false));
                btn.addEventListener('mouseleave', touchEventHandler(false));
            }
        }
    }

    // دالة التحديث التي تعمل في كل إطار (Frame) على جهاز اللاعب
    update() {
        const state = this.session.state;
        if (!state) return; // التأكد من وصول بيانات اللعبة من الخادم

        // مزامنة السيارات: إضافة سيارات اللاعبين الجدد أو حذف سيارات اللاعبين المغادرين
        Object.keys(this.carInstances).forEach(playerId => {
            if (!state.cars[playerId]) {
                scene.remove(this.carInstances[playerId].body);
                delete this.carInstances[playerId];
            }
        });
        Object.keys(state.cars).forEach(playerId => {
        ...
        // تحديث مواقع ودوران كل السيارات في المشهد بسلاسة (Lerp/Slerp)
        ...
        renderer.render(scene, camera); // عرض المشهد المحدث
    }
}

// --- 7. الانضمام إلى جلسة اللعب باستخدام Multisynq ---
...
",
    ...
    debug: []               // يمكن وضع ["writes"] هنا لرؤية بيانات الشبكة في الكونسول للمساعدة في تصحيح الأخطاء
}).then(app => {
    console.log("تم الانضمام بنجاح!");
    const view = app.view || app;
    // حلقة اللعبة الرئيسية (Game Loop) التي تستدعي دالة التحديث باستمرار
    const loop = () => {
        view.update();
